...
class LSTMunit(nn.Module):
    ...
    def __init__(
        self, input_channel_no, hidden_channels_no, kernel_size, stride=1, input_dim=16
    ):
    ...
    def __init__(
        ...
        self.unit_list = nn.ModuleList(
            [
                (
                    LSTMunit(
                        self.full_channel_list[i],
                        self.full_channel_list[i + 1],
                        kernel_size,
                    ).double()
                ).cuda()
                for i in range(len(self.hidden_channel_structure))
            ]
        )
...
class LSTMencdec_onestep(nn.Module):
    ...
    def __init__(self, structure, input_channels, kernel_size=5, debug=True):
        ...
        self.encoder = LSTMmain(
            ...
            hidden_channel_structure=self.enc_shape,
        ...
        self.decoder = LSTMmain(
            ...
            self.kernel_size,
            layer_output=1,
            hidden_channel_structure=self.dec_shape,
...
def analytics(
    ...
    test_model = nn.DataParallel(
        LSTMencdec_onestep(structure, 5, kernel_size=kernel_size, debug=False)
    ).to(
        device
    )  # added data parrallel
